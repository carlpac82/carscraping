...
function showDamageSideSelectionModal() {
    ...
    sides.forEach(side => {
        sidesHTML += `
            <div onclick="selectDamageSide('${side.id}')" style="position: relative; cursor: pointer; transition: transform 0.2s;" 
                 onmouseover="this.style.transform='scale(1.05)';" 
                 onmouseout="this.style.transform='scale(1)';">
                <img src="/static/Inspecçao/${side.image}" alt="${side.label}" style="width: 100%; height: auto; object-fit: contain; display: block; border-radius: 8px;">
                <div style="position: absolute; top: 5px; left: 5px; right: 5px; background: rgba(0,156,182,0.85); color: white; font-weight: bold; font-size: 11px; padding: 4px 6px; border-radius: 4px; text-align: center;">
                    ${side.label}
                </div>
            </div>
        `;
    ...
    const modalHTML = `
        <div id="damageSideModal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.92); z-index: 10000; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; padding: 15px 10px; overflow-y: auto;">
            <h2 style="color: #009cb6; font-size: 20px; font-weight: bold; margin-bottom: 15px; text-align: center; text-shadow: 2px 2px 4px rgba(0,0,0,0.5);">
                Registar Novo Dano
            </h2>
            
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; max-width: 100%; width: 100%; margin-bottom: 15px;">
                ${sidesHTML}
            </div>
            
            <button onclick="closeDamageSideModal()" style="padding: 10px 25px; background: rgba(108, 117, 125, 0.9); color: white; border: 2px solid white; border-radius: 8px; font-size: 14px; font-weight: bold; cursor: pointer; transition: background 0.2s;" 
