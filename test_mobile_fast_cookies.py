#!/usr/bin/env python3
"""
Teste FINAL: Mobile + Cookies Rápidos
"""
import sys
import time
...
print("=" * 80, flush=True)
print("TESTE FINAL - MOBILE + COOKIES RÁPIDOS", flush=True)
print("=" * 80, flush=True)
...
try:
    ...
    # ACEITAR COOKIES RAPIDAMENTE
    print("\n🍪 Aceitando cookies RAPIDAMENTE...", flush=True)
    time.sleep(0.3)  # Só 0.3 segundos
    accept_cookies_fast()
    time.sleep(0.5)
    # Retry
    accept_cookies_fast()
    ...
    for attempt in range(2):
        ...
        if result.get('allFilled'):
            ...
            print(f"   ⚠️  Tentativa {attempt + 1}", flush=True)
            if accept_cookies_fast():
                time.sleep(1)
            ...
            if len(articles) > 0:
                ...
                for i, art in enumerate(articles[:3], 1):
                    try:
                        ...
                        print(f"  {i}. [Erro ao extrair]", flush=True)
        elif "war=" in url:
