#!/usr/bin/env python3
"""
Script para importar Janeiro 2026 CORRETAMENTE
"""
import os
import psycopg2
import pandas as pd
...
def import_january_correct():
    print("=" * 80)
    print("IMPORTAÇÃO CORRETA DE JANEIRO 2026")
    print("=" * 80)
    ...
    try:
        ...
        # Remover todos os dados de Janeiro 2026
        cursor.execute("DELETE FROM broker_bookings WHERE EXTRACT(MONTH FROM pickup_date) = 1 AND EXTRACT(YEAR FROM pickup_date) = 2026")
        conn.commit()
        print("🧹 Todos os dados de Janeiro 2026 removidos")
        ...
        for idx, row in df.iterrows():
            ...
            if current_broker and current_broker_id:
                ...
                if pd.notna(pickup_date) and pd.notna(days) and total_price is not None and total_price > 0:
                    ...
                    if pickup_date.month == 1 and pickup_date.year == 2026:
                        try:
        ...
        # Resumo final
        print(f"\n" + "=" * 80)
        print("RESUMO FINAL - JANEIRO 2026 CORRETO")
        print("=" * 80)
        print(f"✅ Total de registros importados: {total_imported}")
        print(f"❌ Total de erros: {total_errors}")
        
        # Verificar totais
        cursor.execute('SELECT COUNT(*) FROM broker_bookings WHERE EXTRACT(MONTH FROM pickup_date) = 1 AND EXTRACT(YEAR FROM pickup_date) = 2026')
        total_db = cursor.fetchone()[0]
        print(f"📊 Total de broker_bookings Janeiro 2026: {total_db}")
        
        # Mostrar distribuição
        cursor.execute('''
            SELECT broker_name, COUNT(*) as total, SUM(total_price) as valor_total
            FROM broker_bookings 
            WHERE EXTRACT(MONTH FROM pickup_date) = 1 AND EXTRACT(YEAR FROM pickup_date) = 2026
