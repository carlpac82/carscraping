    ...
    for filename in files:
        ...
        for row_idx in range(1, ws.max_row + 1):
            ...
            if col1_value and not col2_value:
                # Verificar se não é um voucher (números)
                if not str(col1_value).strip().replace('-', '').isdigit():
            ...
            if col2_value and current_commissioner_id:
                # Verificar se tem voucher na coluna 1
                voucher = None
                ...
                # Gerar voucher único se não existir
                if not voucher:
                    voucher = f"COMM-2025-{voucher_counter:06d}"
                    voucher_counter += 1
                
                # Inserir em commission_bookings SEM dropoff_date e horas
                try:
                    cur.execute("""
                        INSERT INTO commission_bookings (
                            voucher_number, client_name, client_email, client_phone,
                            pickup_date, price, commissioner_id, commission_rate, 
                            commission_amount, commission_paid, status, created_at, 
                            updated_at, pickup_location, dropoff_location, vehicle_group
                        ) VALUES (
                            %s, %s, %s, %s, %s, %s, %s, %s, %s, false, 'confirmed',
                            NOW(), NOW(), 'N/A', 'N/A', 'N/A'
                        )
                    """, (
                        voucher,
