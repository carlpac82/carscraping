...
async def _send_booking_confirmed_email(booking_data: dict):
    """Send booking confirmation email to client in their language"""
    try:
        lang = booking_data.get('language', 'pt')
        client_email = booking_data.get('client_email', '')
        voucher_num = booking_data.get('voucher_number', '')
        print(f"[CONFIRM EMAIL] Sending confirmation to {client_email} lang={lang}")

        i18n = {
            'pt': {
                'subject_line': f"Reserva Confirmada - {voucher_num}",
                'greeting': 'Olá',
                'confirmed_banner': 'Reserva Confirmada!',
                'confirmed_intro': 'A sua reserva foi confirmada. Estamos à sua espera! Abaixo encontra os detalhes da sua reserva.',
                'summary_title': 'Resumo da Reserva',
                'group_label': 'GRUPO',
                'pickup_label': 'Levantamento',
                'dropoff_label': 'Devolução',
                'bring_title': 'Não se esqueça de trazer:',
                'bring_license': 'Carta de Condução válida',
                'bring_passport': 'Passaporte ou Cartão de Cidadão / ID Card',
                'bring_card': 'Cartão de Crédito (em nome do condutor principal)',
                'checkin_title': 'Check-in Online',
                'checkin_text': 'Poupe tempo na chegada — faça o seu check-in online antes de vir buscar o carro.',
                'checkin_btn': 'Fazer Check-in Online',
                'checkin_url_label': 'Ou aceda em',
                'maps_label': 'Ver no Google Maps',
                'footer_auto': 'Este email foi gerado automaticamente pelo sistema de reservas da Auto Prudente.',
            },
            'en': {
                'subject_line': f"Booking Confirmed - {voucher_num}",
                'greeting': 'Hello',
                'confirmed_banner': 'Booking Confirmed!',
                'confirmed_intro': 'Your booking has been confirmed. We look forward to welcoming you! Below you will find your booking details.',
                'summary_title': 'Booking Summary',
                'group_label': 'GROUP',
                'pickup_label': 'Pickup',
                'dropoff_label': 'Return',
                'bring_title': 'Please remember to bring:',
                'bring_license': 'Valid Driving Licence',
                'bring_passport': 'Passport or National ID Card',
                'bring_card': 'Credit Card (in the name of the main driver)',
                'checkin_title': 'Online Check-in',
                'checkin_text': 'Save time on arrival — complete your online check-in before picking up the car.',
                'checkin_btn': 'Complete Online Check-in',
                'checkin_url_label': 'Or visit',
                'maps_label': 'View on Google Maps',
                'footer_auto': 'This email was automatically generated by the Auto Prudente booking system.',
            },
            'fr': {
                'subject_line': f"Réservation Confirmée - {voucher_num}",
                'greeting': 'Bonjour',
                'confirmed_banner': 'Réservation Confirmée !',
                'confirmed_intro': 'Votre réservation a été confirmée. Nous avons hâte de vous accueillir ! Vous trouverez ci-dessous les détails de votre réservation.',
                'summary_title': 'Résumé de la Réservation',
                'group_label': 'GROUPE',
                'pickup_label': 'Départ',
                'dropoff_label': 'Retour',
                'bring_title': 'N\'oubliez pas d\'apporter :',
                'bring_license': 'Permis de conduire valide',
                'bring_passport': 'Passeport ou Carte d\'identité nationale',
                'bring_card': 'Carte de crédit (au nom du conducteur principal)',
                'checkin_title': 'Check-in en Ligne',
                'checkin_text': 'Gagnez du temps à l\'arrivée — effectuez votre check-in en ligne avant de venir chercher le véhicule.',
                'checkin_btn': 'Faire le Check-in en Ligne',
                'checkin_url_label': 'Ou visitez',
                'maps_label': 'Voir sur Google Maps',
                'footer_auto': 'Cet email a été généré automatiquement par le système de réservation Auto Prudente.',
            },
            'de': {
                'subject_line': f"Buchung Bestätigt - {voucher_num}",
                'greeting': 'Hallo',
                'confirmed_banner': 'Buchung Bestätigt!',
                'confirmed_intro': 'Ihre Buchung wurde bestätigt. Wir freuen uns auf Ihren Besuch! Unten finden Sie Ihre Buchungsdetails.',
                'summary_title': 'Buchungsübersicht',
                'group_label': 'GRUPPE',
                'pickup_label': 'Abholung',
                'dropoff_label': 'Rückgabe',
                'bring_title': 'Bitte denken Sie daran, Folgendes mitzubringen:',
                'bring_license': 'Gültiger Führerschein',
                'bring_passport': 'Reisepass oder Personalausweis',
                'bring_card': 'Kreditkarte (auf den Namen des Hauptfahrers)',
                'checkin_title': 'Online Check-in',
                'checkin_text': 'Sparen Sie Zeit bei der Ankunft — führen Sie Ihren Online-Check-in vor der Fahrzeugabholung durch.',
                'checkin_btn': 'Online Check-in durchführen',
                'checkin_url_label': 'Oder besuchen Sie',
                'maps_label': 'Auf Google Maps anzeigen',
                'footer_auto': 'Diese E-Mail wurde automatisch vom Buchungssystem der Auto Prudente generiert.',
            },
        }
        ci = i18n.get(lang, i18n['en'])

        tpl_path = os.path.join(os.path.dirname(__file__), 'templates', 'email_booking_confirmed.html')
        with open(tpl_path, 'r', encoding='utf-8') as f:
            tpl = f.read()
        html_content = Template(tpl).render(**{**booking_data, **ci, 'language': lang})

        credentials = get_gmail_credentials()
        if not credentials:
            print(f"[CONFIRM EMAIL] No Gmail credentials")
            return False
        ...
        mo = _MM()
        mo['to'] = client_email
        mo['subject'] = ci['subject_line']
        mo.attach(_MT(html_content, 'html'))
        msg = {'raw': _b64.urlsafe_b64encode(mo.as_bytes()).decode()}

        service = build('gmail', 'v1', credentials=credentials)
        result = service.users().messages().send(userId='me', body=msg).execute()
        print(f"[CONFIRM EMAIL] Sent to {client_email}, ID: {result.get('id')}")
        return True
